...
public:
    ...
    void SetUp() {
        ...
        vmac1 = std::make_shared<VMAC_StreamHasher>(VMAC_StreamHasher::buffer(key1), nonce.data());

        auto key2 = new CryptoPP::FixedSizeAlignedSecBlock<byte,16>();
        std::memcpy(key2->data(), std::string("abcdefghijklmnop").data(), 16);
        vmac2 = std::make_shared<VMAC_StreamHasher>(VMAC_StreamHasher::buffer(key2), nonce.data());
    }
...
TEST_F(VMAC_StreamHasherTest, BadMessage) {
    ...
    hasher(tag.data(), reinterpret_cast<const byte*>(m.data()), m.size(),0);
    std::string s("bad message");
    ASSERT_FALSE(verifier(tag.data(), reinterpret_cast<const byte*>(s.data()), s.size(),0)) << "The tag was wrongly done as good";
}

TEST_F(VMAC_StreamHasherTest, TestVector0) {
    auto hasher = vmac1->Hasher();
    auto verifier = vmac2->Verifier();
    hasher(tag.data(), reinterpret_cast<const byte*>(m.data()), 0, 0);
    ASSERT_EQ(std::memcmp(tag.data(), rfc_tag[0].data(), 8), 0) << "The tag for message " << m << " is wrong";
    ASSERT_TRUE(verifier(tag.data(), reinterpret_cast<const byte*>(m.data()), 0, 0)) << "Can not verify the tag for message " << m;
}
...
TEST_F(VMAC_StreamHasherTest, TestVector1) {
    ...
    ASSERT_TRUE(verifier(tag.data(), reinterpret_cast<const byte*>(m.data()), m.size(), 0)) << "Can not verify the tag for message " << m;
}
...
TEST_F(VMAC_StreamHasherTest, TestVector2) {
    ...
    auto s = m;
    multiply(s, 15);
    hasher(tag.data(), reinterpret_cast<const byte*>(s.data()), s.size(), 0);
    ASSERT_EQ(std::memcmp(tag.data(), rfc_tag[2].data(), 8), 0) << "The tag for message " << m << " is wrong";
    ASSERT_TRUE(verifier(tag.data(), reinterpret_cast<const byte*>(s.data()), s.size(), 0)) << "Can not verify the tag for message " << m;
}
